...
export function serveAccount(measureCtx: MeasureContext, brandings: BrandingMap, onClose?: () => void): void {
    ...
    //     const { account, extra } = decodeTokenVerbose(measureCtx, token)
    //     console.log("check account, exta:>>>>", account, extra)
    //     let email: string | undefined
    //     let plan = 'cloud:free'

    //     try {
    //         // const [db] = await accountsDb
    //         // const user = await db.person.findOne({ uuid: account })

    //         if (extra && extra.email) {

    //             // email = extra.email as string
    //             // console.log("check email:>>>>", email)
    //             // const res = await fetch(`https://iam.hanzo.ai/api/get-user?email=${encodeURIComponent(email)}`, {
    //             //     headers: { accept: 'application/json' }
    //             // })

    //             // const json = await res.json()
    //             // console.log("check response:>>>>", json)
    //             // if (json.status === 'ok' && Array.isArray(json.data?.plans)) {
    //             //     const plans = json.data.plans.map((p: any) => p.plan)
    //             //     const priority = ['cloud:free', 'cloud:dev', 'cloud:team', 'cloud:pro']
    //             //     plan = [...priority].reverse().find(p => plans.includes(p)) ?? 'cloud:free'
    //             //     console.log("check plan:>>>>", plan)
    //             // }
    //         } else {
    //             console.log("token không có mail")
    //         }
    //     } catch (err) {
    //         console.error('Failed to fetch user or plan from IAM:', err)
    //     }
    ...
    router.get('/me', async (ctx) => {
        ...
        const { account, extra } = decodeTokenVerbose(measureCtx, token)
        console.log("check account, extra:>>>>", account, extra)
        ...
        try {
            if (extra?.email) {
                email = extra.email as string
                const res = await fetch(`https://cloud.hanzo.ai/api/public/organization?email=${encodeURIComponent(email)}`, {
                    headers: { accept: 'application/json' }
                })
                ...
                if (json.user?.organizations && Array.isArray(json.user.organizations)) {
                    const plans = json.user.organizations.map((org: any) => org.plan)
                    const priority = ['cloud:free', 'cloud:dev', 'cloud:team', 'cloud:pro']
                    plan = [...priority].reverse().find(p => plans.includes(p)) ?? 'cloud:free'
                }
            } else {
            ...
            console.error('Failed to fetch plan from cloud.hanzo.ai:', err)
        }
