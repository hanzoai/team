...
export async function toggleReaction (message: Message, emoji: string): Promise<void> {
  ...
  if (reaction !== undefined) {
    await communicationClient.removeReaction(message.card, message.id, emoji, new Date())
  } else {
    await communicationClient.createReaction(message.card, message.id, emoji, new Date())
  }
}
